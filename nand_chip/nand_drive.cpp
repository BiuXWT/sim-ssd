...
int NandDrive::erase_block(const Addr *addr)
{
    if (!addr)
        return -1;
    if (addr->die >= DIE_CNT || addr->plane >= PLANE_CNT || addr->block >= BLOCK_CNT)
        return -1;
...
int NandDrive::write_page(const Addr *addr, const uint8_t *main)
{
    if (!addr)
        return -1;
    if (addr->die >= DIE_CNT || addr->plane >= PLANE_CNT || addr->block >= BLOCK_CNT || addr->page >= PAGE_CNT)
        return -1;
    if (!main)
        return -1;
...
int NandDrive::addr_valid(const Addr *addr)
{
    ...
    if (addr->die >= DIE_CNT)
        return 0;
    if (addr->plane >= PLANE_CNT)
        return 0;
    if (addr->block >= BLOCK_CNT)
        return 0;
    if (addr->page >= PAGE_CNT)
        return 0;
    return 1;
}
